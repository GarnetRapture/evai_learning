set_project("evai_kernels")
set_xmakever("3.1.1")
add_rules("mode.release")
add_rules("plugin.compile_commands.autoupdate", {outputdir = path.join(os.scriptdir(), "..", "..", "..", ".vscode")})

target("evai_kernels_device")
    set_kind("static")
    add_toolchains("cuda")
    set_languages("c++20")
    set_runtimes("MD")
    set_values("cuda.rdc", false)
    add_files("sr_adamw_kernels.cu", "rms_norm_kernels.cu", "swiglu_kernels.cu", "rope_kernels.cu")
...
target("evai_kernels_cuda")
    set_kind("shared")
    add_deps("evai_kernels_device")
    ...
    add_cxxflags(
        "/Zc:__cplusplus", "/Zc:preprocessor", "/utf-8", "/external:anglebrackets", "/external:W0",
        "/wd4267", {force = true})
