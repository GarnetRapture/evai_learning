"""Fixed identity-regression evaluation for the persona spirit model (plan.md SS14).

Evaluation compares the base model and the trained persona model on the same
fixed prompts per persona, scoring whether generic assistant behavior has
resurfaced and whether the persona's own identity, speech, and relationships
remain intact. `PERSONA_CROSS_CONTAMINATION` additionally checks that a
persona never claims another registered persona's name as its own (a
requirement the user added on top of plan.md SS14's original 11 categories).
"""
...
class EvaluationCategory(StrEnum):
    """Categories for assessing persona fidelity and catching generic assistant regression."""
...
class RegressionEvaluationPrompt:
    """Fixed evaluation prompt for detecting persona drift or assistant resurgence."""
...
class EvaluationMetric:
    """A named evaluation score with supporting commentary."""
...
class EvaluationReport:
    """Report summarizing persona adapter evaluation versus base model."""
...
def build_fixed_regression_prompts(
    ...
    """Build the fixed, persona-specific regression prompt set for all 12 categories.

    One prompt per category, evaluated identically on the base model and the
    trained model (plan.md SS14). `other_persona_names` supplies at least one
    other registered persona's name for the PERSONA_CROSS_CONTAMINATION probe.
    """
...
def detect_degenerate_output(response: str) -> str | None:
    """Detect an empty or repetition-collapsed response (undertrained model symptom).

    Returns a short reason string when the response is empty or when a
    single word/character dominates it, otherwise None. This is not a
    coherence judge; it only catches the specific failure mode where
    generation collapses into a repeated token (e.g. "???...", "그냥 그냥 그냥...").
    """
...
def score_response_against_prompt(
    ...
    """Deterministically score one model response against its fixed regression prompt.

    This is a conservative keyword-based net, not a semantic judge: a hit on
    a forbidden pattern, a generic cross-category assistant-leakage phrase,
    or a degenerate (empty/repetition-collapsed) response scores 0.0,
    otherwise 1.0. `details` always carries the raw response so a human
    reviewer can verify borderline cases.
    """
...
def run_regression_evaluation(
    ...
    """Run the fixed regression prompt set through `generate_fn` and score every response.

    `generate_fn(prompt: str) -> str` is the caller's model-inference
    callable (kept generic so this module never imports torch/transformers
    directly; that import belongs to the CLI/train layer that owns the
    loaded model).
    """
