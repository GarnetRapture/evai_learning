...
from .paths import ADAPTERS_DIR, DATASETS_DIR, MERGED_DIR, MODEL_DIR
...
def load_base_and_full_state(
    ...
    if unclassified:
        raise FFFError(f"Parameters not covered by the adapter layout: {unclassified}")
    if set(base) != set(full):
        raise FFFError(f"Parameter key mismatch between base and '{persona_id}' checkpoint")
    return base, full
...
def _read_split_messages(persona_id: str, split: str) -> list[list[dict[str, str]]]:
    path = DATASETS_DIR / persona_id / f"{split}.jsonl"
    if not path.exists():
        return []
...
def measure_rank_fidelity(
    ...
    if not conversations:
        raise FFFError(f"No SFT records available to measure fidelity for '{persona_id}'")
    batches = [
    ...
    model = AutoModelForCausalLM.from_pretrained(
        str(MODEL_DIR), local_files_only=True, dtype=torch.float32, device_map=device
    )
    model.eval()
    if model.lm_head.weight.data_ptr() != model.model.embed_tokens.weight.data_ptr():
        raise FFFError("Expected lm_head to share storage with embed_tokens (tie_word_embeddings)")
    ...
    def load_into_model(state: dict[str, torch.Tensor]) -> None:
        ...
        if unexpected or missing:
            raise FFFError(f"State load mismatch: unexpected={unexpected} missing={missing}")
...
def load_untied_base_model(dtype: torch.dtype) -> Any:
    ...
    model = AutoModelForCausalLM.from_pretrained(str(MODEL_DIR), local_files_only=True, dtype=dtype)
    embed = model.get_input_embeddings()
    head = model.get_output_embeddings()
    if not isinstance(embed, torch.nn.Embedding) or not isinstance(head, torch.nn.Linear):
        raise FFFError("Base model input/output embeddings are not Embedding/Linear modules")
    if head.weight.data_ptr() != embed.weight.data_ptr():
        raise FFFError("Base model is expected to load with tied input/output embeddings")
    head.weight = torch.nn.Parameter(embed.weight.detach().clone())
    model.config.tie_word_embeddings = False
    if head.weight.data_ptr() == embed.weight.data_ptr():
        raise FFFError("Failed to untie lm_head from embed_tokens")
    return model
...
def extract_spirit_adapter(persona_id: str, rank: int, output_root: Path | None = None) -> Path:
    ...
    if not tied_head_assigned:
        raise FFFError(f"lm_head LoRA was not created for '{persona_id}'")
...
class SpiritRuntime:
    def __init__(self, persona_ids: list[str], adapters_root: Path | None = None) -> None:
        ...
        if not persona_ids:
            raise FFFError("SpiritRuntime requires at least one persona adapter")
    ...
    def activate(self, persona_id: str) -> None:
        if persona_id not in self.model.peft_config:
            raise FFFError(f"Spirit adapter '{persona_id}' is not loaded in this runtime")
    ...
    def generate(self, user_message: str, max_new_tokens: int = 128) -> str:
